...
bool kbc_able_to_read(){    // see BIT(5)
    uint8_t stat;
    if (util_sys_inb(KBC_ST_REG, &stat) != OK) return false;
    if ((stat & KBC_OBF) == 0){
        //printf("Outbuffer not full\n");
        return false;    // mouse isn't filling the output buffer when testing manually, idk why
    }
    if ((stat & (KBC_PAR_ERR | KBC_TO_ERR)) != OK){
        printf("Error in Out buffer\n");
        return false;
    }
    //if ((stat & KBC_AUX) == 0) return false; // this bit checks if it's from the mouse
    return true;
}
